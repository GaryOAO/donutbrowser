...
/// Fire-and-forget: queue an immediate sync for the profile if (and only if)
/// sync is enabled for it. The on-disk encrypted dir changed, so the sync
/// engine needs to re-upload it. No-op when sync isn't configured or the
/// profile has sync disabled, mirroring the upstream eligibility gate.
fn queue_profile_sync_if_eligible(profile: &crate::profile::BrowserProfile) {
...
pub async fn set_profile_password(profile_id: String, password: String) -> Result<(), String> {
  ...
  cache_key(id, key);
  queue_profile_sync_if_eligible(&profile);
